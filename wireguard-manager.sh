...
\tprefetch-key: yes"
  ...
  function wireguard-setconf() {
    ...
    if [ "${INSTALL_UNBOUND}" == true ]; then
      NFTABLES_POSTUP="sysctl -w net.ipv4.ip_forward=1; sysctl -w net.ipv6.conf.all.forwarding=1; nft add table inet wireguard-${WIREGUARD_PUB_NIC}; nft add chain inet wireguard-${WIREGUARD_PUB_NIC} wireguard_chain {type nat hook postrouting priority srcnat\;}; nft add rule inet wireguard-${WIREGUARD_PUB_NIC} wireguard_chain oifname ${SERVER_PUB_NIC} masquerade"
    ...
    9) # Update the script
      ...
      if [ -x "$(command -v unbound)" ]; then
        ...
        if [ -f "${UNBOUND_CONFIG_HOST}" ]; then
          ...
          NEW_UNBOUND_HOSTS_HASH=$(curl --silent ${UNBOUND_CONFIG_HOST_URL} | openssl dgst -sha3-512 | cut -d " " -f2)

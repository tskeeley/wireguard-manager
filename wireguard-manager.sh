...
\tprefetch-key: yes"
  ...
  function wireguard-setconf() {
    ...
    if [ "${INSTALL_UNBOUND}" == true ]; then
      NFTABLES_POSTUP="sysctl -w net.ipv4.ip_forward=1; sysctl -w net.ipv6.conf.all.forwarding=1; nft add table inet wireguard-${WIREGUARD_PUB_NIC}; nft add chain inet wireguard-${WIREGUARD_PUB_NIC} wireguard_chain {type nat hook postrouting priority 100\;} nft add rule inet wireguard-${WIREGUARD_PUB_NIC} wireguard_chain oifname ${SERVER_PUB_NIC} masquerade"
